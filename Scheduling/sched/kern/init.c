...
#define TESTED(x) (__builtin_strcmp(#x, STRING(TEST)) == 0)
	...
	// Ejemplo de uso con tasks que disminuyen la prioridad al minimo, en
	// conjunto con tareas de CPU no intensivo
	for (int i = 0; i < 8; i++) {
		ENV_CREATE(user_hello, ENV_TYPE_USER);
	}
	ENV_CREATE(user_prioritysyscalls, ENV_TYPE_USER);


	// Ejemplo de uso con tasks que usan CPU intensivamente, en conjunto con tareas de CPU no intensivo
